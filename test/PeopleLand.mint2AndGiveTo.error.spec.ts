...
describe("PeopleLand.mint2AndGiveTo.error", async () => {
  it("no role", async () => {
    ...
    await expect(
      landNFTToken
        .connect(w2)
        .mint2AndGiveTo(111, 111, w3.address, 112, 112, w4.address,{ value: BigNumber.from(10).pow(18).mul(2) })
  ...
  it("mint2AndGiveTo three", async () => {
    ...
    await (
      await landNFTToken
        .connect(w1)
        .mint2AndGiveTo(111, 111, w2.address, 112, 112, w3.address,{ value: BigNumber.from(10).pow(18).mul(2) })
    ).wait();

    await expect(
      landNFTToken
        .connect(w1)
        .mint2AndGiveTo(113, 113, w4.address, 114, 114, w5.address,{ value: BigNumber.from(10).pow(18).mul(2) })
    ).to.revertedWith("caller is already minted");

    await (
      await landNFTToken
        .connect(w2)
        .mint2AndGiveTo(115, 115, w4.address, 116, 116, w5.address,{ value: BigNumber.from(10).pow(18).mul(2) })
    ).wait();
  ...
  it("mint2AndGiveTo reserved", async () => {
    ...
    for(let i = 0; i < data.length; i++) {
      ...
      await expect(
        landNFTToken
          .connect(w1)
          .mint2AndGiveTo(xy[0], xy[1], w2.address, 100, 100, w3.address, { value: BigNumber.from(10).pow(18).mul(2) })
